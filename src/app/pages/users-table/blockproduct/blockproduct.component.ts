...
@Component({
  selector: "ngx-blockproduct",
  templateUrl: "./blockproduct.component.html",
  styleUrls: ["./blockproduct.component.scss"]
})
export class BlockproductComponent {
  // settings = {
  //   actions: {
  //     add: false
  //   },
  ...
  view(data) {
    this.router.navigate([
      "/pages/users-tables/blockproducts-activity",
      data.product_id
    ]);
  }
  Unblockuser() {
    if (this.arr.length !== 0) {
      if (window.confirm("Are you sure you want to UNBLOCK?")) {
        this.service.unblockproduct_mutli(this.arr).subscribe(
          res => {
            console.log(res);
            if (res.success === true) {
              location.reload();
  ...
  ngOnInit(): void {
    this.service.getProdcutsblock().subscribe(
      res => {
        this.tableShow = true;
