...
@Component({
  selector: "ngx-blocksupplier",
  templateUrl: "./blocksupplier.component.html",
  styleUrls: ["./blocksupplier.component.scss"]
})
export class BlocksupplierComponent implements OnInit {
  // settings = {
  //   actions: {
  //     add: false
  //   },
  ...
  view(data) {
    this.router.navigate([
      "/pages/users-tables/blocksuppliers-activity",
      data.vendor_id
    ]);
  }
  Unblockuser() {
    if (this.arr.length !== 0) {
      if (window.confirm("Are you sure you want to UNBLOCK?")) {
        this.service.unblocksupplier_multi(this.arr).subscribe(
          res => {
            console.log(res);
            if (res.success === true) {
              location.reload();
  ...
  // }
  ngOnInit(): void {
    this.service.getblockvendorData().subscribe(
      res => {
        this.tableShow = true;
