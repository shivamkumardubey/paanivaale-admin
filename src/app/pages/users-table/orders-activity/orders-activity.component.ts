...
@Component({
  selector: "ngx-orders-activity",
  templateUrl: "./orders-activity.component.html",
  styleUrls: ["./orders-activity.component.scss"]
})
export class OrdersActivityComponent {
  constructor(
    ...
    //   }
    // );
    this.service.getdata_ordersbyid(this.route.snapshot.params.id).subscribe(
      res => {
        this.orderdata = res;
        this.suplliervendor();
        this.users();
  ...
  assignuser(id) {
      ...
      this.service
        ...
        .subscribe(
          res => {
            ...
            this.router.navigate(["/pages/users-tables/orders"]);
          },
          error => {
            console.log(error);
            alert(error.error.message);
    ...
    if (this.deliverydata) {
      ...
      this.service
        .assignorder(this.route.snapshot.params.id, id.target.value, "Vendor")
        .subscribe(
          res => {
            if (res.success) {
              alert("Order Assigned");
