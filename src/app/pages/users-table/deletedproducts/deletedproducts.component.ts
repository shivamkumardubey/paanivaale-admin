...
@Component({
  selector: "ngx-deletedproducts",
  templateUrl: "./deletedproducts.component.html",
  styleUrls: ["./deletedproducts.component.scss"]
})
export class DeletedproductsComponent {
  ...
  view(data) {
    this.router.navigate([
      "/pages/users-tables/deletedproducts-activity",
      data.product_id
    ]);
  }
  undelete() {
    if (this.arr.length !== 0) {
      if (window.confirm("Are you sure you want to UNDELETE?")) {
        this.service.undeleteproduct_mutli(this.arr).subscribe(
          res => {
            console.log(res);
            if (res.success === true) {
              location.reload();
  ...
  ngOnInit(): void {
    this.service.getProdcutsdeleted().subscribe(
      res => {
        this.tableShow = true;
