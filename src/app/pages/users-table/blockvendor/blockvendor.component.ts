...
@Component({
  selector: "ngx-blockvendor",
  templateUrl: "./blockvendor.component.html",
  styleUrls: ["./blockvendor.component.scss"]
})
export class BlockvendorComponent {
  // settings = {
  //   actions: {
  //     add: false
  //   },
  ...
  view(data) {
    this.router.navigate([
      "/pages/users-tables/blockvendors-activity",
  ...
  Unblockuser() {
    if (this.arr.length !== 0) {
      if (window.confirm("Are you sure you want to UNBLOCK?")) {
        this.service.unblockvendor_multi(this.arr).subscribe(
          res => {
            console.log(res);
            if (res.success === true) {
              location.reload();
  ...
  ngOnInit(): void {
    this.service.getdeliveryblockdata().subscribe(
      res => {
        this.tableShow = true;
