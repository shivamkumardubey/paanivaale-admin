...
@Component({
  selector: "ngx-deletedvendor",
  templateUrl: "./deletedvendor.component.html",
  styleUrls: ["./deletedvendor.component.scss"]
})
export class DeletedvendorComponent {
  // settings = {
  //   actions: false,
  //   mode: "external",
  //   add: {
  ...
  view(data) {
    this.router.navigate([
      "/pages/users-tables/deletedvendors-activity",
  ...
  undelete() {
    if (this.arr.length !== 0) {
      if (window.confirm("Are you sure you want to UNDELETE?")) {
        this.service.undeletevendor_multi(this.arr).subscribe(
          res => {
            console.log(res);
            if (res.success === true) {
              location.reload();
  ...
  ngOnInit(): void {
    this.service.getdeliverydeleteddata().subscribe(
      res => {
        this.tableShow = true;
